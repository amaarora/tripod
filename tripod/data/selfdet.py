...
import torch
import numpy as np
import tripod.data.transforms as T
...
def get_random_patch_from_img(img, min_pixel=8):
    ...
    x, y = (
        np.random.randint(w - sw) if sw != w else 0,
        np.random.randint(h - sh) if sh != h else 0,
    )
...
def build_selfdet(image_set, args):
    root = Path(args.imagenet_path)
    assert root.exists(), f"provided ImageNet path {root} does not exist"
    PATHS = {
        "train": root / "train",
        "val": root / "val",
    }
